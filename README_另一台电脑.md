...
3. 确认数据目录存在且原始文件在：
   ...
   ```
   里面需有 `2026\` 各船文件夹（每个船一个文件夹、含最新 Excel）。
   **注意：旧的 `CUL DAILY MOVEMENT.xlsx`（旧大Excel）已不再需要，不用管它。脚本生成的结果都放在 `更新\生成结果\` 子文件夹里（大Excel 与 PIC 表），不是根目录。**
...
- **改 PIC**：打开 Z 盘 `更新\生成结果\PIC汇总.xlsx`，只改 "PIC" 列，其余别动；保存后跑 bat 自动生效。
