...
function onDecomToggle(cb){
  ...
  applyDecomFilter();
  refreshFullGrouping();   // 重建 lane 定位 + vessel 分组底色（Full Schedule 交替色带）
  // 同步两个工具栏的勾选框 + 视觉指示
...
/* 重建 Lane 定位 + vessel 分组底色。fullData 变化（已下线筛选）或 Lane 顺序变化后调用 */
function refreshFullGrouping(){
  buildVesselLaneRank();
  // 按默认排序后的顺序分配交替底色，保证视觉上相邻船连续换色
  var seen = {}, gi = 0;
  defaultFullSort(fullData.slice()).forEach(function(r){
...
function exportFullScheduleExcel(wb){
  ...
  // Sort exactly like the UI default view: vessel clusters anchored to their
  // next-voyage lane (defaultFullSort). A vessel serving several lanes keeps
  // ALL its rows in ONE section; the Route column still shows each row's lane.
  // (旧逻辑按 lane→船名 重排，跨航线的船在 Excel 里被拆成多节 —— 2026-09-10 用户确认改掉)
  var exportData = defaultFullSort(data.slice());
  ...
  for(var i=0;i<exportData.length;i++){
    ...
    if(r.vessel!==prevVessel){
      ...
      // Title Lane = 该船的锚定 lane（与页面聚簇位置一致）；单航线船即其航线
      var anchorLane=(r.vessel in vesselLaneRank) ? (getLaneOrder()[vesselLaneRank[r.vessel]]||r.route||'') : (r.route||'');
      var titleText='Lane: '+anchorLane+'    Vessel: '+(r.vessel||'')+'    Code: '+(r.code||'')+'    PIC: '+(r.pic||'');
      var titleRow=[];
...
var EXPORT_TABLES = [
  ...
  {group:'Port Wait', items:[
    ...
    {id:'portWaitVvdTable', label:'VVD Call Detail (every call behind the filtered ports)', sheet:'Port Wait VVD', rich:'vvd'},
  ]},
  {group:'BOA', items:[
  ...
  {group:'Maintenance', items:[
    {id:'maintPortTable', label:'Maintenance by Port (aggregated)', sheet:'Maint by Port'},
    ...
    {id:'maintVvdTable', label:'VVD Call Detail (every call behind the filtered ports)', sheet:'Maint VVD', rich:'maintvvd'},
  ]},
];
...
// ── Port Wait VVD detail sheet ────────────────────────────────────────
// The Port Wait main table only exports its aggregated rows (one row per
// port). This sheet flattens EVERY individual call behind those ports:
// one row per VVD call, so the user can drill into which vessel/voyage
// drove a port's wait / berth rate.
//   - Respects the current Port Wait filters (date range, port filter,
...
function exportPortWaitVvdExcel(wb){
  ...
  // Pre-build ONE style set per band (even/odd) and reuse it for every row —
  // per-row allocation would otherwise create tens of thousands of style
  // objects and slow the xlsx serialiser right down.
  function rowStyles(fc){
    return {
      txt   :{font:{name:'Arial',sz:9},fill:F(fc),border:B,alignment:A('left','center')},
      ctr   :{font:{name:'Arial',sz:9},fill:F(fc),border:B,alignment:A('center','center')},
      wait  :{font:{name:'Arial',sz:9},fill:F(fc),border:B,alignment:A('center','center'),numFmt:'0.0'},
      stOk  :{font:{name:'Arial',sz:9,bold:true,color:{rgb:'1E7145'}},fill:F(fc),border:B,alignment:A('center','center')},
      ...
      vvdGap:{font:{name:'Arial',sz:9,bold:true,color:{rgb:'9C5700'}},fill:F('FFF2CC'),border:B,alignment:A('left','center')}
    };
  }
  var SE=rowStyles('EBF3FB'), SO=rowStyles('FFFFFF');

  var sheetData=[];
  var tr=[]; for(var c=0;c<numCols;c++) tr[c]={v:(c===0?titleText:''),s:tS};
  sheetData.push(tr);
  sheetData.push(headers.map(function(h){return{v:h,s:hS};}));
  ...
  rows.forEach(function(r,i){
    ...
    var row=[
      ...
      {v:(r.code||r.vessel||'')+(r.voy||''), s:(isStdVesselCode(r.code)?S.vvdOk:S.vvdGap)},
      {v:r.eta,    s:S.ctr},
      {v:r.etb,    s:S.ctr},
      {v:r.etd,    s:S.ctr},
  ...
  XLSX.utils.book_append_sheet(wb,ws,'Port Wait VVD');
  return true;
}

// ── Maintenance VVD detail sheet ────────────────────────────────────
// Mirrors the Port Wait VVD sheet: one row per maintenance call behind the
// CURRENT Maintenance filters, so the aggregated "Maint by Port" rows can be
// drilled into. Needed because _tabSheetFromDom() skips `detail-wrap` rows —
// i.e. the on-page "All calls (N)" sub-tables were never exported before.
//
// NOTE (differs from Port Wait!): MAINT_RECORDS.vessel is ALREADY the 4-char
// vessel code (100% of 24.9k rows), and voyage + dir are SEPARATE columns, so
//   VVD = vessel + voyage + dir      e.g. BFCP + 2526 + E -> BFCP2526E
// No code lookup table and no amber gap flag is needed here.
function exportMaintVvdExcel(wb){
  var recs = [];
  ...
  recs.forEach(function(r){
    if(r.plog==='Y') nPlog++;
    if(r.vsched===1) nVs++;
    if(r.port) ports[r.port]=true;
  });
  var titleText='CUL DAILY MOVEMENT — Maintenance VVD Call Detail  —  '+Object.keys(ports).length+
                ' ports · '+recs.length+' calls · Port Log '+maintPct(nPlog,recs.length).toFixed(1)+'% maintained'+
                (VSCHED_AVAILABLE ? (' · Vessel Sched '+maintPct(nVs,recs.length).toFixed(1)+'% maintained') : '')+
                '  —  As of '+TODAY_DATA.date;
  ...
  // Pre-build ONE style set per band (even/odd) and reuse it for every row.
  // Allocating ~8 objects per row would create 200k+ style objects at the
  // 24.9k-row "operator = ALL" case and make the xlsx serialiser crawl.
  function rowStyles(fc){
    return {
      txt  :{font:{name:'Arial',sz:9},fill:F(fc),border:B,alignment:A('left','center')},
      ctr  :{font:{name:'Arial',sz:9},fill:F(fc),border:B,alignment:A('center','center')},
      // Unmaintained is the actionable state — red bold on a pale red band.
      plogY:{font:{name:'Arial',sz:9,color:{rgb:'1E7145'}},fill:F(fc),border:B,alignment:A('center','center')},
      plogN:{font:{name:'Arial',sz:9,bold:true,color:{rgb:'C00000'}},fill:F('FDF2F2'),border:B,alignment:A('center','center')},
  ...
  // Deterministic order: port asc → ETD asc → vessel → voyage
  var rows=recs.slice().sort(function(a,b){
    return String(a.port||'').localeCompare(String(b.port||''))
        ...
        || String(a.vessel||'').localeCompare(String(b.vessel||''))
        || String(a.voyage||'').localeCompare(String(b.voyage||''));
  });
  ...
  rows.forEach(function(r,i){
    ...
    sheetData.push([
      ...
      {v:r.voyage||'',                      s:S.ctr},
      {v:r.dir||'',                         s:S.ctr},
      {v:(r.vessel||'')+(r.voyage||'')+(r.dir||''), s:S.txt},
      {v:r.operator||'',                    s:S.ctr},
      {v:r.etd||'',                         s:S.ctr},
  ...
  ws['!autofilter']={ref:'A2:'+XLSX.utils.encode_col(numCols-1)+totalRows};
  ws['!freeze']={xSplit:0,ySplit:2};
  XLSX.utils.book_append_sheet(wb,ws,'Maint VVD');
  return true;
}
...
function exportSelectedTables(){
  _loadXlsx(function(){
    ...
    flat.forEach(function(it){
      ...
      else if(it.rich==='vvd'){ if(exportPortWaitVvdExcel(wb)) count++; }
      else if(it.rich==='maintvvd'){ if(exportMaintVvdExcel(wb)) count++; }
...
function moveLane(idx, dir){
  ...
  saveLaneOrder();
  refreshFullGrouping();          // lane 序变了 → 重算每艘船的主 lane + 交替底色
...
// R1–R10 不再单独成类，而是在 classifyRemark() 开头先按 R_CODE_MAP 映射到语义类别。
// ── R1–R10 → 语义类别映射（按 reason 本质归类，用户 2026-09-01 确认）──────────
//   R1  = Force majeure: port congestion                     → congestion
//   R2  = Force majeure: port closed                         → portclosure
//   R3  = Change terminal to TCHP (CAT LAI severe congestion)→ congestion
//   R4  = Omission (CAT LAI congestion seriously)            → congestion
//   R5  = Force majeure: sea conditions & weather            → weather
//   R6  = Force majeure: terminal equipment malfunction      → equipment
//   R7  = Force majeure: terminal operation efficiency low   → berthops
//   R8  = Change terminal to NBCT P2 (P3 severe congestion)  → congestion
//   R9  = COR to call SHA then NGB, P/O RBC1                 → phase
//   R10 = NGB shift to NBCT P2 (port congestion)             → congestion
// 组合码（R1/R5、R4; R10）取第一个 R 码的映射 —— 主因通常写在前面。
var R_CODE_MAP = {
  R1:'congestion', R2:'portclosure', R3:'congestion', R4:'congestion',
  R5:'weather',    R6:'equipment',   R7:'berthops',   R8:'congestion',
  R9:'phase',      R10:'congestion'
};

var REMARK_CATEGORIES = [
  // 1. 潮汐/水深（不含裸 tide —— "Strong tide make previous vessel working delay" 属塞港）
  {key:'tide',        label:'Tide / Draft',
   keywords:['潮水','tide limit','潮位','吃水','draft limit','low water','水深']},
  // 2. 节假日（纯节日；封港走 portclosure，不含 port close 关键词以免误抓天气封港）
  ...
  // 4. 封港 / 港口关闭（R2）。keywords 留空 —— 只由 R2 映射进入，避免误抓 "Port Closed due to fog"（实为天气）
  {key:'portclosure', label:'Port Closure (Force Majeure)', keywords:[]},
  // 5. 天气/海况/不可抗力（R5）
  {key:'weather',     label:'Weather / Sea Conditions',
  ...
  // 7. 加油/物料供应。注意：不能用裸 'cst'（会误抓 CST 航线名），改用 "380CST" 牌号形态
  ...
  // 11. 航线调整（P/I、P/O、OMIT、SLIDE、改靠、COR）（R9 映射至此）
...
// 纯 R 码形式：整句只由 R# 与分隔符组成（"R1"、"R1/R5"、"R4; R10"、"R9/R1/R2"）
...
function classifyRemark(remark){
  ...
  if(rm){
    var mapped=R_CODE_MAP['R'+parseInt(rm[1],10)];
    if(mapped) return mapped;
    return 'other';           // 未知 R 码（如 R11）
  }
  // ② 否则按语义类顺序匹配，先命中先归
...
function renderMonthlyTrend(){
  ...
  html+='<span>BOA Rate: <b style="color:'+berthRateColor(overallRate)+';">'+overallRate+'%</b></span>';
